...
<head>
    ...
    <title>Buat Password Baru - SSO Engine</title>
        ...
        <div class="w-full max-w-[440px]">
            <div class="text-center mb-xl flex flex-col items-center">
                <div class="flex items-center gap-3 mb-2">
                    <span class="material-symbols-outlined text-[48px] text-secondary">password</span>
                    <h1 class="font-headline-xl text-headline-xl text-primary tracking-tight">Buat <span class="text-secondary">Password Baru</span></h1>
                </div>
            </div>
            ...
            <div class="glass-panel rounded-xl p-xl login-card-shadow">
                <form action="<?= site_url('reset-password/process') ?>" method="post" class="space-y-md">
                    <?= csrf_field() ?>
                    <input type="hidden" name="token" value="<?= esc($token) ?>">
                    <input type="hidden" name="email" value="<?= esc($email) ?>">
                    
                    <div class="space-y-xs">
                        <label class="font-label-sm text-label-sm text-on-surface-variant block uppercase tracking-wider" for="password">Password Baru</label>
                    ...
                    <div class="space-y-xs">
                        <label class="font-label-sm text-label-sm text-on-surface-variant block uppercase tracking-wider" for="password_confirm">Konfirmasi Password Baru</label>
                        <div class="relative group">
                            <div class="absolute inset-y-0 left-0 pl-md flex items-center pointer-events-none text-on-surface-variant group-focus-within:text-secondary transition-colors">
